...
export function middleware(request: NextRequest, event: NextFetchEvent) {
  ...
  // Osiris is now an internal adapter runtime owned by RamiBot. Keep its
  // browser application closed; only the API routes are consumed by the
  // RamiBot gateway. This prevents a second public UI from drifting from the
  // production control plane.
  if (!url.startsWith('/api/')) {
    return new NextResponse(JSON.stringify({
      error: 'OSIRIS_UI_DISABLED',
      message: 'Osiris adapters are available through the RamiBot gateway.',
    }), {
      status: 404,
