...
from world_feeds import WORLD_TOOLS, WorldFeedService
...
class MCPClient:
    def __init__(self):
        ...
        self.world_feeds = WorldFeedService()
    ...
    async def call_tool(self, server_name: str, tool_name: str, arguments: dict) -> dict:
        if server_name == "ramibus-world":
            return await self.world_feeds.call_tool(tool_name, arguments)
        conn = self.servers.get(server_name)
        if not conn:
            raise ValueError(f"Server '{server_name}' not found")
        ...
        all_tools = [
            {
                "type": "function",
                "function": {
                    "name": f"ramibus-world__{tool['name']}",
