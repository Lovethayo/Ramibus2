...
from agent_roster import build_roster, route_request
...
def get_adapter(provider: str):
    ...
    # Local GGUF servers are long-lived processes. Keep one adapter instance so
    # its two model slots remain available for hot swapping and role routing.
    if provider == "local_gguf":
        if provider not in _adapter_instances:
            _adapter_instances[provider] = adapter_cls(**provider_settings)
...
async def lifespan(app: FastAPI):
    ...
    # Load optional Docker settings. Replit shell remains the default runtime.
    settings = load_settings()
    ...
    try:
        ...
        # Seed rami-kali MCP server only when Docker actually responds. The
        # existing MCP architecture is preserved, but Docker is not silently
        # treated as available on Replit.
        ...
        if docker_ok:
            try:
                ...
                docker_ok = False
        if docker_ok and not any(s["name"] == RAMIKALI_NAME for s in servers):
            try:
    ...
    local_adapter = _adapter_instances.get("local_gguf")
    if local_adapter and hasattr(local_adapter, "shutdown"):
        await local_adapter.shutdown()
...
def _resolve_model(provider: str, model: str | None, message: str, team_mode: str) -> tuple[str | None, dict]:
    route = route_request(message, team_mode)
    if provider == "local_gguf":
        requested = (model or "").strip()
        if requested not in {"rami-planner", "rami-tradecraft"}:
            requested = "rami-planner" if route["model_role"] == "planner" else "rami-tradecraft"
...
async def runtime_status():
    """Expose truthful runtime capabilities to the control plane."""
    shell = os.environ.get("SHELL") or shutil.which("bash") or shutil.which("sh")
    docker_available = bool(shutil.which("docker"))
    docker_responding = False
    if docker_available:
        try:
            docker_responding = (await asyncio.to_thread(
    ...
    return {
        "terminal": {
            "mode": "replit",
            "available": bool(shell and Path(shell).exists()),
            "workspace": str(WORKSPACE_ROOT),
            "operator_approval_required": True,
        },
        "docker": {
            "installed": docker_available,
            "responding": docker_responding,
            "available": docker_responding,
        },
    }


@app.get("/api/world/source-health")
async def world_source_health():
    return mcp_client.world_feeds.source_health()


@app.get("/api/world/recent")
async def world_recent(limit: int = Query(50, ge=1, le=500)):
    return await mcp_client.world_feeds.recent(limit)


@app.get("/api/world/search")
async def world_search(query: str = Query(""), limit: int = Query(50, ge=1, le=500)):
    return await mcp_client.world_feeds.search(query, limit)


@app.get("/api/world/near")
async def world_near(
    latitude: float = Query(..., ge=-90, le=90),
    longitude: float = Query(..., ge=-180, le=180),
    radius_km: float = Query(250, gt=0, le=20000),
    limit: int = Query(50, ge=1, le=500),
):
    return await mcp_client.world_feeds.nearby(latitude, longitude, radius_km, limit)


@app.get("/api/world/cctv")
async def world_cctv(query: str = Query(""), limit: int = Query(100, ge=1, le=1000)):
    return await mcp_client.world_feeds.osiris.query("cctv", {"query": query, "limit": limit})


@app.get("/api/world/cctv/near")
async def world_cctv_near(
    latitude: float = Query(..., ge=-90, le=90),
    longitude: float = Query(..., ge=-180, le=180),
    radius_km: float = Query(25, gt=0, le=2000),
    limit: int = Query(100, ge=1, le=1000),
):
    return await mcp_client.world_feeds.cctv.nearby(latitude, longitude, radius_km, limit)


@app.get("/api/world/cctv/{camera_id}")
async def world_cctv_get(camera_id: str):
    camera = await mcp_client.world_feeds.cctv.get(camera_id)
    if not camera:
        raise HTTPException(status_code=404, detail="CCTV camera not found")
    return camera


@app.get("/api/osiris/capabilities")
async def osiris_capabilities():
    """Return the unified Osiris adapter inventory without exposing its UI."""
    return mcp_client.world_feeds.osiris.capabilities()


@app.get("/api/osiris/health")
async def osiris_health():
    """Return truthful direct-feed and internal-bridge health."""
    return mcp_client.world_feeds.osiris.health()


@app.post("/api/osiris/batch")
async def osiris_batch(body: dict):
    queries = body.get("queries", []) if isinstance(body, dict) else []
    if not isinstance(queries, list) or len(queries) > 32:
        raise HTTPException(status_code=400, detail="queries must be an array with at most 32 items")
    return await mcp_client.world_feeds.osiris.batch_query(queries)


@app.get("/api/osiris/query")
async def osiris_query(
    request: Request,
    capability: str = Query(..., min_length=1),
):
    # Query parameters are intentionally collected only after the capability
    # name. This preserves the upstream adapter's input names while keeping
    # the public surface on one authenticated/approved RamiBot gateway.
    params = dict(request.query_params) if request else {}
    params.pop("capability", None)
    try:
        return await mcp_client.world_feeds.osiris.query(capability, params)
    except ValueError as error:
        raise HTTPException(status_code=400, detail=str(error))
    except RuntimeError as error:
        raise HTTPException(status_code=503, detail=str(error))
    except Exception as error:
        raise HTTPException(status_code=502, detail=f"Osiris adapter failed: {error}")
...
async def agents_roster(limit: int = Query(1200, ge=0, le=1200)):
    return {
        "total": 1200,
        "returned": min(limit, 1200),
        "routing": "deterministic role ownership",
        "agents": build_roster(limit),
    }


@app.get("/api/agents/route")
async def agents_route(message: str = Query(...), team_mode: str = Query("red")):
...
async def chat(body: ChatRequest):
    ...
    model, route = _resolve_model(provider, model, body.message, body.team_mode)
    adapter = get_adapter(provider)
...
async def terminal_start(body: TerminalStartRequest):
    ...
    if mode == "replit" and not body.approved:
        raise HTTPException(status_code=400, detail="Operator approval is required to open the Replit shell")
    target = body.container or get_docker_container() or "replit-shell"
    session_id, error, info, shell = await create_session(target, mode=mode)
    if error:
        raise HTTPException(status_code=400, detail=error)
    return {"session_id": session_id, "info": info, "shell": shell, "mode": mode, "workspace": str(WORKSPACE_ROOT)}
