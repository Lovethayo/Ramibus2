"""Replit-shell and Docker terminal SSE bridge — cross-platform.

Replit shell:    real host PTY via pty.openpty() + the workspace shell.
                 This is the default runtime in Replit/Linux userspace.

Docker mode:     real PTY via pty.openpty() + ``docker exec -it`` when
                 Docker is explicitly configured and available.
...
def _detect_replit_shell() -> str:
    """Select a safe shell executable available in the Replit workspace."""
    configured = os.environ.get("SHELL", "")
    candidates = [configured, "bash", "zsh", "sh"]
    for candidate in candidates:
        if not candidate:
            continue
        executable = candidate if os.path.isabs(candidate) else shutil.which(candidate)
        if executable and SHELL_RE.match(executable):
            return executable
    return "/bin/sh"
...
async def _validate_and_prepare(target: str, mode: str) -> tuple[str | None, str | None]:
    """Run all pre-flight checks.

    Returns (shell, None) on success, or (None, error_message) on failure.
    """
    if mode == "replit":
        shell = _detect_replit_shell()
        if not Path(shell).exists():
            return None, "Error: No usable Replit workspace shell is available."
        return shell, None

    if mode != "docker":
        return None, f"Error: Unsupported terminal mode '{mode}'."
    ...
    if not await _container_exists(target):
        return None, f"Error: Container '{target}' not found."

    started_msg = None
    if not await _container_running(target):
        if not await _start_container(target):
            return None, "Failed to start container."
        started_msg = "started"

    shell = await _detect_shell(target)
...
class TerminalSession:
    ...
    def __init__(self, session_id: str, target: str, shell: str, mode: str):
    ...
    def start_pipe(self, use_script: bool = False):
        """Start the configured shell with pipes (Windows).
        ...
        self.proc = subprocess.Popen(
            ...
            cwd=str(WORKSPACE_ROOT) if self.mode == "replit" else None,
            env=os.environ.copy() if self.mode == "replit" else None,
        )
    ...
    def start_pty(self):
        """Start the configured shell with a real PTY (Unix only)."""
        ...
        command = [self.shell, "-i"] if self.mode == "replit" else [
            "docker", "exec", "-it", self.target, self.shell,
        ]
...
async def create_session(target: str, mode: str = "replit") -> tuple[str | None, str | None, list[str], str]:
    ...
    """
    info: list[str] = []
    shell, status = await _validate_and_prepare(target, mode)
    if shell is None:
        # status is the error message
        return None, status, info, ""

    if status == "started":
        info.append(f"Container '{target}' was stopped — started it.")

    session_id = uuid.uuid4().hex[:12]
    session = TerminalSession(session_id, target, shell, mode)

    if IS_UNIX:
        session.start_pty()
    else:
        use_script = mode == "docker" and await _has_script(target)
